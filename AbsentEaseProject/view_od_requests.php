...
$stmt = $conn->prepare("
    SELECT student_rollno, day_and_hours, comment, submission_date, od_dates, faculty_verifier, faculty_verifier_status, link
    FROM od_requests 
    ...
    <?php else: ?>
        <table>
            ...
            foreach ($requests as $row):
                $faculty_verifier = trim(strtolower($row['faculty_verifier']));
                $status_raw = strtolower($row['faculty_verifier_status']);

                if ($faculty_verifier === 'none') {
                    $status_display = '-';
                    $status_color = 'black';
                } else {
                ...
                <tr>
                    ...
                    <td>
                        <?php if (!empty($row['link'])): ?>
                            <a href="<?= htmlspecialchars($row['link']) ?>" target="_blank" style="color: #007bff; text-decoration: underline;">
                                View Proof
                            </a>
                        <?php else: ?>
                            <em style="color: gray;">No Link</em>
                        <?php endif; ?>
                    </td>
                    <td>
