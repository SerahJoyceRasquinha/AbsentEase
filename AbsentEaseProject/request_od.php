...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($check_stmt->num_rows > 0) {
        ...
        $faculty_verifier = $_POST['faculty_verifier'] ?? null;
        $stmt = $conn->prepare("INSERT INTO od_requests (student_rollno, day_and_hours, od_dates, faculty_verifier, status, comment, class_teacher_username) VALUES (?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("sssssss", $student_rollno, $selected_hours, $od_dates_str, $faculty_verifier, $status, $comment, $class_teacher_username);
        $stmt->execute();
        $stmt->close();
...
<div class="container">
    ...
    <form method="POST" onsubmit="return prepareData();">
        ...
        <select name="faculty_verifier" required>
            <option value="">-- Select Faculty Verifier --</option>
...
<script>
    ...
    function prepareData() {
        ...
        if (!verifier) {
            alert("Please select a faculty verifier.");
            return false;
        }
