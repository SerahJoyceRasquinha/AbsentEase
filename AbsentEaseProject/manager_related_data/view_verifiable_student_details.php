...
if (isset($_POST['refresh_students'])) {
    $students = $conn->query("SELECT student_rollno FROM verifiable_student_details");
    $rollnos_to_delete = [];
    ...
    while ($row = $students->fetch_assoc()) {
        ...
        // Build semester durations
        $semesters = [];
        $months = [["june", "december"], ["january", "may"]];
        $sem_no = 1;
        $curr_year = $year_start;

        for ($i = 0; $i < 6; $i++) {
            if ($i % 2 == 0) {
                $semesters[] = [$sem_no, strtotime("june $curr_year"), strtotime("december $curr_year")];
            } else {
                $semesters[] = [$sem_no, strtotime("january " . ($curr_year + 1)), strtotime("may " . ($curr_year + 1))];
                $curr_year++;
            }
            $sem_no++;
        }
        ...
        $current_time = time();
        $current_semester = 0;
        foreach ($semesters as $sem_info) {
            list($sem_number, $start, $end) = $sem_info;
            if ($current_time >= $start && $current_time <= $end) {
                $current_semester = $sem_number;
                break;
            }
        }
        ...
        if ($current_semester == 0) {
            // Past all semesters
            $current_semester = 0;
            $rollnos_to_delete[] = $rollno;
        } else {
            // Update semester
            $update = $conn->prepare("UPDATE verifiable_student_details SET student_current_semester = ? WHERE student_rollno = ?");
            $update->bind_param("is", $current_semester, $rollno);
            $update->execute();
            $update->close();
        }
    }

    // Now delete students who are no longer in college
    if (!empty($rollnos_to_delete)) {
        ...
        if ($stmt1 && $stmt2) {
            ...
            echo "<script>alert('Refresh complete: Removed " . count($rollnos_to_delete) . " graduated students.');</script>";
        } else {
            echo "<script>alert('Error in deletion query.');</script>";
        }
    } else {
        echo "<script>alert('Refresh complete: No graduated students found.');</script>";
    }
}
